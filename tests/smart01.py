...
# Listas para acumular figuras y rutas
figs = []
paths = []
...
for pos in tqdm(range(window, len(df)), desc="Generando frames"):
    ...
    # Acumular figura y path destino
    figs.append(fig)
    paths.append(f"{frames_dir}/frame_{pos:04d}.png")


# Exportar todas las imágenes de una sola vez
print("🖼️ Exportando imágenes...")
try:
    pio.write_images(fig=figs, file=paths)
    print(f"✅ Frames PNG guardados en: {frames_dir}/")
    print(f"📊 Total de frames generados: {len(figs)}")
except Exception as e:
    print(f"[ERROR] Error al exportar imágenes: {e}")
print(f"✅ Frames PNG guardados en: {frames_dir}/")
